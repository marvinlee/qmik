...
(function($, define) {
	...
	function js_beautify(js_source_text, options) {
		...
		var opt_indent_level = options['indent_level'] || 0; // starting
		// indentation
		...
		function print_space() {
			...
			if (last_output !== ' ' && last_output !== '\n' && last_output !== indent_string) { // prevent
				// occassional
				// duplicate
				// space
				output.push(' ');
		...
		function get_next_token() {
			var n_newlines = 0;
			if (parser_pos >= input.length) { return [
				'', 'TK_EOF'
			]; }
			...
			if (c === "'" || // string
					c === '"'
					|| // string
					(c === '/' && ((last_type === 'TK_WORD' && last_text === 'return') || (last_type === 'TK_START_EXPR' || last_type === 'TK_END_BLOCK'
		...
		parser_pos = 0;
		in_case = false; // flag for parser that case/default has been processed,
		...
		while (true) {
			...
			case 'TK_WORD':
				...
				if (last_type !== 'TK_END_BLOCK' && in_array(token_text.toLowerCase(), [
					'else', 'catch', 'finally'
				])) {
					print_newline();
