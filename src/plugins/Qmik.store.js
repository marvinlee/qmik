...
; (function (Q) {
    ...
    var store = {
        ...
        setSession: function (key, value, ttl) {//储存到sessionStorage,ttl:unit is second
            ttl = ttl || -1;
            var item = { __data: value, __ttl: ttlTime(ttl) };
            session && session.setItem(key, JSON.stringify(item))
        },
        setCookie: function (key, value, ttl, domain) {//储存到cookie,ttl:unit is second
