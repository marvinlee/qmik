 ...
 */
(function($) {
	var doc = document, win = window, loc = location, regLongDate = /[ ]*[\d]{4}-[\d]{1,2}-[\d]{1,2}[ ][\d]{1,2}:[\d]{1,2}:[\d]{1,2}[ ]*/, regTime = /[ ]*[\d]{1,2}:[\d]{1,2}:[\d]{1,2}[ ]*/, regShortDate = /[ ]*[\d]{4}-[\d]{1,2}-[\d]{1,2}[ ]*/, regFmt = /(((y|Y){2,4}-M{1,2}-d{1,2}[ ])?h{1,2}:m{1,2}:s{1,2})|((y|Y){2,4}-M{1,2}-d{1,2}[ ](h{1,2}:m{1,2}:s{1,2})?)/, regYear = /(y|Y){2,4}/, dfmt = "yyyy-MM-dd hh:mm:ss";
	...
	$.extend( {
		util : {
			...
			isIphone : function() {
				return /i(Phone|P(o|a)d)/.test(navigator.userAgent) && typeof win.ontouchstart !== 'undefined'
			},
			isAndroid : function() {
			...
			isSupport3D : function() {
				try {
					return $.util.isIphone && 'WebKitCSSMatrix' in win && 'm11' in new WebKitCSSMatrix()
				} catch (e) {
					return false
				}
			},
			selectTxt : function(target, flag) {// 是否可选文本
				if ($.isNull(flag) || flag) $(target).css( {
					"-moz-user-select" : "",
					"-webkit-user-select" : "",
					"-o-user-select" : "",
					"user-select" : ""
